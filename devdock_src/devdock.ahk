...
ControlSource = txt\MenuDock.txt
...
reset:
	...
	Loop, PARSE, keys, |
	{
		...
		PressedKeys("Delete", A_LoopField)
	}
;clear ctl variables
Loop, %CtlMax% {
VarSetCapacity(Ctl%A_Index%, 0)
VarSetCapacity(Ctl%A_Index%_UpKey, 0)
VarSetCapacity(Ctl%A_Index%_DownKey, 0)
VarSetCapacity(togglekey%A_Index%_pressed, 0)
}
...
CreateGui:
	...
	ifWinActive, Blend
{
; Set the canvas size for this program
; this assumes you have your taskbar set to autohide, which is of course terrible coding, as this should auto adjust :) feel free to help out with that!
	w = 1283
	h = 693
	; MsgBox, blend found
	theFile =  txt\blend.txt
	ControlSource = txt\blend.txt
	Run programming.exe, C:\Program Files (x86), max
	
; take windows title bar off of BLEND, fit app to dock size
	...
	ifWinActive, Sublime
{
	w = 1273
	h = 683
	; MsgBox, sublime found
	theFile =  txt\MenuDock.txt
	ControlSource = txt\MenuDock.txt
	Run programming.exe, C:\Program Files (x86), max
	
; take windows title bar off of Sublime, fit app to dock size
  WinGet Style, Style, A
  WinGetPos, X, Y, Width, Height, A
  WinSet, Style, -0xC40000, A
  WinMove,A,,0,0,w,h	
...
{
	;MsgBox, photoshop found
	theFile =  txt\photoshop.txt
	ControlSource = txt\photoshop.txt
	}	
	
	ifWinActive ahk_class Illustrator
{
	;MsgBox, photoshop found
	theFile =  txt\illustrator.txt
	ControlSource = txt\illustrator.txt
	}		
	ifWinActive ahk_class != Photoshop && ifWinActive != Blend && ifWinActive != Sublime|| && ifWinActive ahk_class != Illustrator
{
	; MsgBox, blend, submlime, and photoshop NOT found
	theFile =  txt\MenuDock.txt
	ControlSource = txt\MenuDock.txt
	}

	If (Argument1) and (ControlSource = "txt\MenuDock.txt")
...
If (GuiColor)
	...
	Loop, %CtlMax% {
		...
		If (Ctl%A_Index%_Type = "zoom") {
			controltype := "Picture"
			text :=  Ctl%A_Index%_Image1
			option .= "  0xE hwndhPic"
			zoom := Ctl%A_Index%_Zoom
			zoomx := Ctl%A_Index%_X
			zoomy := Ctl%A_Index%_Y
			zoomw := Ctl%A_Index%_W
			zoomh := Ctl%A_Index%_H
			zooma := Ctl%A_Index%_Align
			zoomf := Ctl%A_Index%_follow
	...
	;original Gui, +AlwaysOnTop  +ToolWindow -Caption +E0x8000000 +LastFound
...
;		WinSet, TransColor, %GuiTransColor%
	...
	If (InStr(GuiPos, "Left"))
		if (guidocking) {
		GuiX := 0 ;gahfe original areaLeft
		} else {
		GuiX :=areaLeft
		}
...
If (InStr(GuiPos, "autov")) {
        if (a_Caret("y","screen")>((areaBottom-areaTop)/2))
	GuiY := areaTop
	else
        GuiY := areaBottom - GuiHeight
}
...
Guiwidth:=Guiwidth-30
Guimini:=GuiWidth+30
WinSet, Region, 0-0 %Guimini%-0 %Guimini%-30 %GuiWidth%-30 %GuiWidth%-%GuiHeight% 0-%GuiHeight%, ahk_id %GuiHwnd%
;end test
 */
...
touchblockgui:
if (guitouchgate=0)
ChangeControlView(CtlToggletouch_ID, 1)
;gahfe test
if (guistat=1) {
Gui, 1:Destroy
guistat=2
...
; rest of the elements are initialised in sub-routine RegisterAppBar
...
uEdge=0 ; left=0,top=1,right=2,bottom=3

RegisterAppBar:
  Off := &APPBARDATA+12
  Off :=  NumPut( uEdge, Off+0 )
  Off :=  NumPut(  GX, Off+0 )
  Off :=  NumPut(  GY, Off+0 )
  Off :=  NumPut(  GW, Off+0 )
  Off :=  NumPut(  GH, Off+0 )
...
;run magnifier.ahk %zoomh% %zoomw% %zoomx% %zoomy%
  hdd_frame := DllCall("GetDC", UInt, PrintSourceID)
  hdc_frame := DllCall("GetDC", UInt, hPic)
...
; touchblock code
RegWrite, REG_DWORD, HKEY_CURRENT_USER, Software\Microsoft\Wisp\Touch, TouchGate, %GuiTouchGate%
...
; control: joystick
Ctljoystick_Proc(stat, x, y, cnum)
{
	Global
	Static appmod
	Local ctl, difference
	...
	If (stat) {
		...
		; touch move
		Else {
			differencev := y - VSlider%ctl%_LastPos
			differenceh := x - HSlider%ctl%_LastPos
			If (Abs(differencev) > Ctl%cnum%_Step) {
				;Send, % (differencev < 0) ? Ctl%cnum%%appmod%_Upkey : Ctl%cnum%%appmod%_DownKey
				ControlAction(cnum, (differencev < 0) ? "Up" : "Down", appmod)
				VSlider%ctl%_LastPos := y
				VSlider%ctl%_Moved := True
				Sleep, 100
				ControlAction(cnum, (differencev < 0) ? "Upup" : "Downup", appmod)
			}else {
			If (Abs(differenceh) > (Ctl%cnum%_Step)) {
				;Send, % (differenceh < 0) ? Ctl%cnum%%appmod%_Upkey : Ctl%cnum%%appmod%_DownKey
				ControlAction(cnum, (differenceh < 0) ? "Left" : "Right", appmod)				
				HSlider%ctl%_LastPos := x
				HSlider%ctl%_Moved := True	
				Sleep, 100
				ControlAction(cnum, (differenceh < 0) ? "Leftup" : "Rightup", appmod)	
			}
			}
			; Quit process if there is no receipt of a message for 100ms
			; (measures for dropped messages)
;			SetTimer, CtlHSlider_CheckRelease, 1000
			SetTimer, CtlVSlider_CheckRelease, 1000
;hslider
...
;		SetTimer, CtlHSlider_CheckRelease, Off
		...
		HSlider%ctl%_LastPos := 0
		; short touch
		If (!VSlider%ctl%_Moved && (A_TickCount - VSlider%ctl%_Begin < 200)) {
			ControlAction(cnum, "Touch", appmod)
			Sleep, 100
			ControlAction(cnum, "Touchup", appmod)
;			If (Ctl%cnum%%appmod%_TouchKey
;				Send, % Ctl%cnum%%appmod%_TouchKey
		} 
	}
...
appmod := GetAppMod(cnum)
	...
	; touch up
	Else If (!stat) {
if (Ctl%cnum%%appmod%_Name)
ctl := Ctl%cnum%%appmod%_Name
ControlSource = txt\%ctl%.txt
...
GoSub, reset
;end test
;		if (ControlSource2!=ControlSource) {
;		Run, %A_AhkPath% "%A_ScriptFullPath%" "%ControlSource2%"
;		ExitApp
;		}
		ChangeControlView(cnum, stat)
	}
...
CtlAppDock_Proc(stat, x, y, cnum)
{
	...
	Else If (!stat) {
		...
		local closeid
		closeid := %ctl%procID
		If WinExist("ahk_pid "closeid) {
			;msgbox, %closeid%
			PostMessage, 0x111, 65307,,,ahk_pid %closeid%
			winclose, ahk_pid %closeid%
			;process,close,%closeid%
			;ControlSend,,{alt down}{f4}{alt up},ahk_pid %closeid%
			} else {
			Run, %A_AhkPath% "alt\AppDock.ahk"  "%ControlSourceH%",,,proces_id
			%ctl%procID := proces_id
			}
	}
...
; control: Toggletouch2
CtlToggletouch2_Proc(stat, x, y, cnum, force_release = False)
{
	Global
	Static appmod
	Local ctl
...
; control: ToggleTouch
CtlToggleTouch_Proc(stat, x, y, cnum, force_release = False)
{
	Global
	Static appmod
	Local ctl
	...
	; touch up
	Else If (!stat) {
		RegRead, OutputVar, HKEY_CURRENT_USER, Software\Microsoft\Wisp\Touch, TouchGate
		touchstatus=%OutputVar%
		ControlAction(cnum, "Up", appmod)
		If (Ctl%cnum%_Upkey) {
			;Send, % Ctl%cnum%%appmod%_Upkey 
			PressedKeys("Delete", ctl)
		...
		RegWrite, REG_DWORD, HKEY_CURRENT_USER, Software\Microsoft\Wisp\Touch, TouchGate, 1
		SendMessage, 0x1A,,,, ahk_id 0xFFFF
		ChangeControlView(cnum, 0)
		}
		else
		{
		RegWrite, REG_DWORD, HKEY_CURRENT_USER, Software\Microsoft\Wisp\Touch, TouchGate, 0
		SendMessage, 0x1A,,,, ahk_id 0xFFFF
		ChangeControlView(cnum, 1)
		}
;	MsgBox, Program files are in: %touchstatus%
	}
...
; control: Minimize2
CtlMinimize2_Proc(stat, x, y, cnum)
{
	Global
	Local ctl
	ctl := Ctl%cnum%_Name
	...
	If (stat && !Minimize%cnum%_Pressed) {
		...
		w := Ctl%cnum%_W + ((Ctl%cnum%_X < 0) ? Ctl%cnum%_X : 0) 
		h := Ctl%cnum%_H + ((Ctl%cnum%_Y < 0) ? Ctl%cnum%_Y : 0)
		WinSet, Region, 0-0 w%w% h%h%, ahk_id %GuiHwnd%
		Minimize%cnum%_Pressed := A_TickCount
		GuiMinimizedControl := cnum
	}
	; touch up
...
ControlAction(n, action, appmod)
{
	...
	If (Ctl%n%%appmod%_%action%Key){
		command=% Ctl%n%%appmod%_%action%Key
		Send, %command%
		}
...
WM_DISPLAYCHANGE(wParam, lParam, msg, hwnd)
{
	...
	If (hwnd == GuiHwnd) {
		...
		WinMoveSnap(hwnd, 0, 0, 0)
GoSub, reset
	}
	Return
}

WM_KillFOCUS(wParam, lParam, msg, hwnd){
return
}
...
;MsgBox hola
   CoordMode,  Mouse, Screen
    ;MouseGetPos, mx, my
    if (zooma = "right")
	zoomx:=a_Caret("X","screen") - (zoomw / 2 / zoom) +20
    else
	zoomx:=a_Caret("X","screen") 
    zoomy:=a_Caret("y","screen")  + (zoomh / 2 / zoom) -2
    WinGetPos, wx, wy,,, ahk_id %GuiHwnd%
	;tooltip, y%zoomy% x%zoomx%
    if (zoomx="") or (zoomy="")
	MouseGetPos, zoomx, zoomy


    if(zoomx != zoomxp) OR (zoomy !- zoomyp) 
    {
        ...
        DllCall( "gdi32.dll\StretchBlt"
                ...
                , Int , 2                       ; nXOriginDest
                , Int , 2                       ; nYOriginDest
                , Int , zoomw-6                    ; nWidthDest
                , Int , zoomh-6                    ; nHeightDest
                , UInt, hdd_frame               ; hdcSrc
                , Int , zoomx - (zoomw / 2 / zoom)    ; nXOriginSrc
                , Int , zoomy - (zoomh / 2 / zoom)    ; nYOriginSrc
                , Int , zoomw / zoom               ; nWidthSrc
                , Int , zoomh / zoom               ; nHeightSrc
